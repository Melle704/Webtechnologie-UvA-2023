// Whilst the card display itself has been written by us.
// The mouse interaction here is based on:
//
// `https://codepen.io/jordizle/pen/bGReWV` by Jordan Leigh.
...
function Viewport(data) {
    ...
    var cardWindow = document.getElementsByClassName('card-window')[0];

    bindEvent(cardWindow, 'mousedown', function() {
        self.down = true;
    });

    bindEvent(window, 'mouseup', function() {
        self.down = false;
    });

    bindEvent(cardWindow, 'keyup', function() {
        self.down = false;
    });

    bindEvent(window, 'mousemove', function(e) {
        self.mouseX = e.pageX;
        self.mouseY = e.pageY;
    });
    ...
    bindEvent(cardWindow, 'touchmove', function(e) {
        if (e.preventDefault) {
            e.preventDefault();
        }
    ...
    bindEvent(cardWindow, 'touchend', function(e) {
        self.down = false;
    });
...
var viewport = new Viewport({
    element: document.getElementsByClassName('floating-card')[0],
    fps: 20,
    sensivity: .1,
    sensivityFade: .93,
    speed: 2,
...
function Card(data) {
    ...
    this.element = data.element;
    this.sides = this.element.getElementsByClassName('card-face');
...
Card.prototype.upsideDown = function(obj) {
    var deg = obj.upsideDown ? '180deg' : '0deg';
    var i = 5;

    while (i > 0 && --i) {
        this.sides[i].style['Transform'] = 'rotate(' + deg + ')';
    }
